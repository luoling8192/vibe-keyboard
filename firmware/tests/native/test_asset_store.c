...
static void test_publish_failure_preserves(void){
    ...
    {fs_t f={.mounted=true,.free_bytes=10000};vk_asset_store_t s=make(&f);assert(vk_asset_store_mount(&s)==ESP_OK);publish_simple(&s,1,0);file_t*conflict=newf(&f,"/config/commit-r00000002.vkc");assert(conflict);conflict->size=1;const char*a="{\"revision\":2}",*sc="{\"revision\":2}";vk_asset_revision_t r={2,1,(const uint8_t*)a,strlen(a),(const uint8_t*)sc,strlen(sc)};uint8_t x[32],y[32];assert(vk_asset_store_publish_revision(&s,&r,x,y)==ESP_ERR_INVALID_STATE);assert(s.selected_revision==1);}
}
